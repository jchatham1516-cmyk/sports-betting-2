...
class EloState:
    """
    Simple persistent Elo container with idempotent processing.
    Stored as JSON:
      {
    ...
    def save(self, path: str) -> None:
        ...
        payload = {
            ...
            "processed": sorted(list(self.processed))[:50000],  # safety cap
    ...
    def load(cls, path: str) -> "EloState":
        ...
        try:
            ...
            if payload.get("default_rating") is not None:
                st.default_rating = float(payload.get("default_rating"))
            return st
        except Exception:
