...
from sports.nba.injuries import (
    fetch_official_nba_injuries as fetch_espn_nba_injuries,  # patched import
...
QB_EXTRA_ELO = 10.0  # not used in NBA but kept for compatibility
...
def run_daily_nba(game_date_str: str, *, odds_dict: dict) -> pd.DataFrame:
    ...
    for (home_in, away_in), oi in (odds_dict or {}).items():
        ...
        rows.append({
            ...
            "market_home_prob": float(mkt_home_p) if not np.isnan(mkt_home_p) else
