...
def update_elo_from_recent_scores(days_from: int = 7) -> EloState:
    ...
    for ev in events:
        # Only completed games have scores filled in
        home = ev.get("home_team")
        away = ev.get("away_team")
        scores = ev.get("scores")  # list like [{"name":team,"score":".."}, ...]
        if not home or not away or not scores:
            continue

        # Make a stable unique key
...
def run_daily_nfl(game_date_str: str) -> pd.DataFrame:
    # update Elo from last week of results (keeps it automated)
