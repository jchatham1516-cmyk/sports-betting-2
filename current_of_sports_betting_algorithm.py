...
def main(argv=None):
    ...
    if args.sport == "nba":
        ...
        for matchup, info in (odds_dict or {}).items():
            ...
            commence_dt_utc = commence_dt.astimezone(timezone.utc)
            if window_start <= commence_dt_utc < window_end:
                filtered_odds[matchup] = info
        odds_dict = filtered_odds
        print(
            "[nhl odds] filtered games: "
            f"before={before_count} after={len(odds_dict)} window_start={window_start.isoformat()} "
